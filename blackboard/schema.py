...
class ExecutionMetadata(BaseModel):
    """Metadata about agent execution for visibility and analysis"""
    execution_mode: str = Field(default="sequential", description="'parallel' or 'sequential'")
    agents_executed: List[str] = Field(default_factory=list, description="Agents that were executed")
    execution_times: Dict[str, float] = Field(default_factory=dict, description="Execution time per agent in seconds")
    total_execution_time: float = Field(default=0.0, description="Total execution time in seconds")
    parallel_speedup: Optional[float] = Field(None, description="Speedup factor vs sequential (if parallel)")
...
class BlackboardState(TypedDict):
    ...
    # Execution Metadata (for parallel execution tracking)
    execution_metadata: Optional[Dict[str, Any]]  # ExecutionMetadata as dict
